...
export const postBook = async (props) => {
    return await axios.post(`http://localhost:8080/api/book`, props).then((res) => { console.log('Thêm thành công') }).catch((err) => alert('Đã xảy ra lỗi', err));;
}
...
export const updateBook = async (id, props) => {
    return await axios.put(`http://localhost:8080/api/book/${id}`, props).then((res) => { console.log('Cập nhật thành công') }).catch((err) => alert('Đã xảy ra lỗi', err));;
};

export const deleteBook = async (id) => {
    return await axios.delete(`http://localhost:8080/api/book/${id}`).then((res) => { console.log('Xóa thành công') }).catch((err) => alert('Đã xảy ra lỗi', err));;
};
